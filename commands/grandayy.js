...
exports.run = function(client, message, args) {
    ...
    Jimp.read("./res/watermark.png").then(function (image) { watermark = image.opacity(.75); });
    ...
    Jimp.read(imageurl).then(function (image) {
        ...
        image.composite(watermark, 20, image.bitmap.height - (watermark.bitmap.height + 20)).getBuffer(Jimp.MIME_PNG, (error, buffer) => {
			message.channel.send("", {
				files: [{ attachment: buffer, name: "grandayy.png" }]
			});
        });
    });
}
